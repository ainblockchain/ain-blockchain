...
curl -H "Content-type:application/json" -d '{"jsonrpc": "2.0", "id": "curltest", "method": "ain_getBlockList", "params": {}}' http://localhost:8080/json-rpc > b1.txt
curl -H "Content-type:application/json" -d '{"jsonrpc": "2.0", "id": "curltest", "method": "ain_getBlockList", "params": {}}' http://localhost:8081/json-rpc > b2.txt
