...
**30분에 포함하지 않는 전제 조건**(시험 전날 준비): §3.1 클론·`npm install`·`env.sh`, gpt-oss-20b 로컬 다운로드(약 27 GB), Qwen3.8-Flash-Next 서빙 컨테이너·상주 트레이너·Ainize 노드 기동(§8.2, 모델 적재 약 10분), OpenDART 인증키와 `dart-build-datasets.py` 실행(§8.3, 약 30분 — 샘플링에서는 1종만 쓰므로 `python3 dart-build-datasets.py 8` 대신 미리 만든 `dart-datasets/` 를 두어도 된다).
...
아래는 `$KPI/harness/` 및 `$KPI/` (= 저장소 `tools/cert-kpi/`) 의 실제 파일을 그대로 옮긴 것이다(생성: 2026-09-11 01:57, `$KPI/regen-appendix-h.py`). 본문이 인용하는 유일한 원본이며, 하네스를 수정하면 본 부록을 재생성한다.
