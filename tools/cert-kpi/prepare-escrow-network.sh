...
tracker_port=${TRACKER_PORT:-21079}
[[ "$rpc_port_base" =~ ^[0-9]+$ && "$peer_port" =~ ^[0-9]+$ && "$p2p_port_base" =~ ^[0-9]+$ && "$tracker_port" =~ ^[0-9]+$ ]] || { echo 'Port settings must be numeric' >&2; exit 1; }
(( rpc_port_base >= 1024 && rpc_port_base + 9 <= 65535 && peer_port >= 1024 && p2p_port_base >= 1024 && p2p_port_base + 9 <= 65535 && tracker_port >= 1024 && tracker_port <= 65535 )) || { echo 'Port settings out of range' >&2; exit 1; }
...
for port in "$peer_port" "$tracker_port" $(seq "$rpc_port_base" $((rpc_port_base + 9))) $(seq "$p2p_port_base" $((p2p_port_base + 9))); do
...
node - "$out" "$private" "$project" "$image" "$rpc_port_base" "$peer_port" "$p2p_port_base" "$tracker_port" <<'NODE'
const fs = require('fs');
const [out, privateDirectory, project, chainImage, rpcPortBase, peerPort, p2pPortBase, trackerPort] = process.argv.slice(2);
...
const plan = { ...genesis, project, chainImage, rpcPortBase: Number(rpcPortBase), peerPort: Number(peerPort), p2pPortBase: Number(p2pPortBase), trackerPort: Number(trackerPort), nativeReleaseVersion: 2,
