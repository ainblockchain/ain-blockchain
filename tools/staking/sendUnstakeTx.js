// A tool to unstake AIN tokens for a blockchain app.
const ainUtil = require('@ainblockchain/ain-util');
const CommonUtil = require('../../common/common-util');
const { getAccountPrivateKey, signAndSendTx, confirmTransaction } = require('../util');

function buildUnstakeTxBody(address, appName, ainAmount, timestamp) {
  return {
    operation: {
      type: 'SET_VALUE',
      ref: `/staking/${appName}/${address}/0/unstake/${timestamp}/value`,
      value: ainAmount
    },
    gas_price: 500,
    timestamp,
...
async function sendTransaction(endpointUrl, chainId, appName, ainAmount, account) {
  ...
  const txBody = buildUnstakeTxBody(account.address, appName, ainAmount, timestamp);
...
async function sendUnstakeTx(endpointUrl, chainId, appName, ainAmount, accountType, keystoreFilepath) {
...
async function processArguments() {
  ...
  await sendUnstakeTx(endpointUrl, chainId, appName, ainAmount, accountType, keystoreFilepath);
}

function usage() {
  console.log('\nUsage: node sendUnstakeTx.js <Endpoint Url> <Chain Id> <App Name> <Ain Amount> <Account Type> [<Keystore Filepath>]\n');
  console.log('Example: node sendUnstakeTx.js http://localhost:8081 0 test_app 100 private_key');
  console.log('Example: node sendUnstakeTx.js http://localhost:8081 0 test_app 100 mnemonic');
  console.log('Example: node sendUnstakeTx.js http://localhost:8081 0 test_app 100 keystore keystore_user.json');
