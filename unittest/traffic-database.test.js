...
describe("traffic-database", () => {
  ...
  describe("addEvent", () => {
    it("with intervals not overlapping", () => {
      ...
      assert.deepEqual(tdb.metricCircularQueue, [30, 30, 0, 30, 0, 0, 0, 0, 0, 0]);
      ...
      assert.deepEqual(tdb.metricCircularQueue, [30, 30, 0, 40, 0, 0, 0, 0, 0, 0]);
      // no changes
      expect(tdb.lastIntervalCount).to.equal(3);
      expect(tdb.lastQueueIndex).to.equal(3);
    });
    ...
    it("with intervals overlapping", () => {
      ...
      assert.deepEqual(tdb.metricCircularQueue, [0, 20, 0, 20, 0, 0, 0, 0, 0, 0]);
    ...
    it("with various time", () => {
      ...
      assert.deepEqual(tdb.metricCircularQueue, [0, 10, 10, 0, 0, 0, 0, 0, 0, 0]);
