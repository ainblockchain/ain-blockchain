...
describe("BlockPool", () => {
  ...
  function createAndAddBlock(node, blockPool, lastBlock, number, epoch) {
    const block = Block.create(
        ...
        {[node.account.address]: { stake: 100000, producing_right: true } }, 0, 0);
    const proposal = getTransaction(node, {
        operation: {
          ...
          value: {
            number: block.number,
            epoch: block.epoch,
            validators: { [node.account.address]: { stake: 100000, producing_right: true } },
  ...
  it("Adds blocks to BlockPool", () => {
    ...
    const block = Block.create(
        ...
        {[addr]: { stake: 100000, producing_right: true }}, 0, 0);
    const proposalTx = getTransaction(node1, {
        operation: {
          ...
          value: {
            number: block.number,
            epoch: block.epoch,
            validators: {[addr]: { stake: 100000, producing_right: true } },
            total_at_stake: 100000,
            proposer: addr,
            block_hash: block.hash
          }
