...
FILES_FOR_TRACKER="blockchain/ client/ common/ consensus/ db/ genesis-configs/ logger/ tracker-server/ package.json setup_blockchain_ubuntu.sh start_tracker_gcp.sh"
FILES_FOR_NODE="blockchain/ client/ common/ consensus/ db/ json_rpc/ genesis-configs/ logger/ node/ tx-pool/ p2p/ package.json setup_blockchain_ubuntu.sh start_node_gcp.sh"
...
gcloud compute ssh $TRACKER_TARGET_ADDR --command ". start_tracker_gcp.sh" --project $PROJECT_ID --zone $TRACKER_ZONE
printf "\n\n##########################\n# Starting parent node 0 #\n##########################\n\n"
gcloud compute ssh $NODE_0_TARGET_ADDR --command ". start_node_gcp.sh $SEASON 0 0" --project $PROJECT_ID --zone $NODE_0_ZONE
printf "\n\n##########################\n# Starting parent node 1 #\n##########################\n\n"
gcloud compute ssh $NODE_1_TARGET_ADDR --command ". start_node_gcp.sh $SEASON 0 1" --project $PROJECT_ID --zone $NODE_1_ZONE
printf "\n\n##########################\n# Starting parent node 2 #\n##########################\n\n"
gcloud compute ssh $NODE_2_TARGET_ADDR --command ". start_node_gcp.sh $SEASON 0 2" --project $PROJECT_ID --zone $NODE_2_ZONE
printf "\n\n##########################\n# Starting parent node 3 #\n##########################\n\n"
gcloud compute ssh $NODE_3_TARGET_ADDR --command ". start_node_gcp.sh $SEASON 0 3" --project $PROJECT_ID --zone $NODE_3_ZONE
printf "\n\n##########################\n# Starting parent node 4 #\n##########################\n\n"
gcloud compute ssh $NODE_4_TARGET_ADDR --command ". start_node_gcp.sh $SEASON 0 4" --project $PROJECT_ID --zone $NODE_4_ZONE
...
if [[ "$NUM_SHARDS" -gt 0 ]]; then
    ...
    for i in $(seq $NUM_SHARDS)
        do
            ...
            gcloud compute ssh $SHARD_TRACKER_TARGET_ADDR --command ". start_tracker_gcp.sh" --project $PROJECT_ID --zone $TRACKER_ZONE
            printf "\n\n##########################\n# Starting shard_$i node 0 #\n##########################\n\n"
            gcloud compute ssh $SHARD_NODE_0_TARGET_ADDR --command ". start_node_gcp.sh $SEASON $i 0" --project $PROJECT_ID --zone $NODE_0_ZONE
            printf "\n\n##########################\n# Starting shard_$i node 1 #\n##########################\n\n"
            gcloud compute ssh $SHARD_NODE_1_TARGET_ADDR --command ". start_node_gcp.sh $SEASON $i 1" --project $PROJECT_ID --zone $NODE_1_ZONE
            printf "\n\n##########################\n# Starting shard_$i node 2 #\n##########################\n\n"
            gcloud compute ssh $SHARD_NODE_2_TARGET_ADDR --command ". start_node_gcp.sh $SEASON $i 2" --project $PROJECT_ID --zone $NODE_2_ZONE
        done
fi
