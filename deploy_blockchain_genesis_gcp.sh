...
gcloud compute ssh $NODE_5_TARGET_ADDR --command "$START_NODE_COMMAND_BASE 0 5 $ACCOUNT_INJECTION_OPTION --rest-func" --project $PROJECT_ID --zone $NODE_5_ZONE
inject_account "5"
printf "\n\n##########################\n# Starting parent node 6 #\n##########################\n\n"
gcloud compute ssh $NODE_6_TARGET_ADDR --command "$START_NODE_COMMAND_BASE 0 6 $ACCOUNT_INJECTION_OPTION --rest-func" --project $PROJECT_ID --zone $NODE_6_ZONE
inject_account "6"
